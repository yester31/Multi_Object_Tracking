    ...
    int b_idx, int INPUT_SIZE, int INPUT_H, int INPUT_W)
{
    ...
    */
    ...
    int image_h = rgb_img.rows;
    int image_w = rgb_img.cols;

    // 1. resize 800x800
    cv::Mat resized_img;
    if ((int)INPUT_W != image_w || (int)INPUT_H != image_h){
        cv::resize(rgb_img, resized_img, cv::Size(INPUT_W, INPUT_H));
    }
    else{
        resized_img = rgb_img.clone();
    }
    ...
    const std::vector<std::vector<int>> &color_table, const std::vector<std::string> &class_names)
{
    ...
    {   
        ...
        std::to_string(cls_id) + ", " + str_conf + ", " + class_names[cls_id];
...
void dome_detr_demo()
{
    ...
    const int INPUT_H{ 800 };
    const int INPUT_W{ 800 };
    const int INPUT_C{ 3 };
    const int CLASS_COUNT{ 8 };
    ...
    std::string engine_file_name{ "Dome-M-AITOD_sim" };  // engine file name (engine file will be generated uisng this name)
    std::filesystem::path engine_dir_path = CUR_DIR / "engine" ;// engine directory path (engine file will be generated in this location)
    std::filesystem::path weight_file_path = CUR_DIR / "../ONNX_Generator/Dome-DETR/onnx/Dome-M-AITOD_800x800_sim.onnx" ; // weight file path

    detr_opti_trt dome_detr_trt = detr_opti_trt(BATCH_SIZE, INPUT_H, INPUT_W, INPUT_C, CLASS_COUNT, precision_mode, serialize, gpu_device, engine_dir_path.string(), engine_file_name, weight_file_path.string());
    ...
    for (int i = 0; i < static_cast<int>(ceil(static_cast<float>(num_test_imgs) / BATCH_SIZE)); i++) // batch unit loop
    {
        ...
        for (int b_idx = 0; b_idx < BATCH_SIZE; b_idx++)
        {
            ...
            pre_proc_dome_detr(inputs, ori_img, b_idx, INPUT_SIZE, INPUT_H, INPUT_W);
        ...
        dome_detr_trt.input_data(inputs.data());
        dome_detr_trt.run_model();
        dome_detr_trt.output_data(outputs.data());
        ...
        for (int b_idx = 0; b_idx < BATCH_SIZE; b_idx++)
        {
            ...
            draw_bbox_text_dome_detr(detection_ptr, img, conf_thre, img_name, save_dir_path, COLOR_TABLE, AITOD_LABELS);
