...
void dfine_demo()
{
    ...
    for (int i = 0; i < static_cast<int>(ceil(static_cast<float>(num_test_imgs) / BATCH_SIZE)); i++) // batch unit loop
    {
        ...
        for (int b_idx = 0; b_idx < BATCH_SIZE; b_idx++)
        {
            ...
            pre_proc_dfine(inputs, ori_img, b_idx, INPUT_SIZE, INPUT_H, INPUT_W);
        ...
        for (int b_idx = 0; b_idx < BATCH_SIZE; b_idx++)
        {
            ...
            draw_bbox_text_dfine(detection_ptr, img, conf_thre, img_name, save_dir_path, COLOR_TABLE, COCO_LABELS);
