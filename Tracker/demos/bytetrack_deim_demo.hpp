...
void bytetrack_deim_demo() {
    ...
    detr_opti_trt deim_trt = detr_opti_trt(BATCH_SIZE, INPUT_H, INPUT_W, INPUT_C, CLASS_COUNT, precision_mode, serialize, gpu_device, engine_dir_path.string(), engine_file_name, weight_file_path.string());
    ...
    std::filesystem::path save_file_path = save_dir_path / "bytetrack_deim_demo.mp4" ;
	...
	while (true)
    {
        ...
        deim_trt.input_data(inputs.data());
        deim_trt.run_model();
        deim_trt.output_data(outputs.data());
